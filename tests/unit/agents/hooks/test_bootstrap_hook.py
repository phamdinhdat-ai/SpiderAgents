...
def hook(working_dir):
    ...
    from openspider.agents.hooks.bootstrap import BootstrapHook
...
class TestBootstrapHookCallEarlyExit:
    ...
    async def test_returns_none_when_not_first_interaction(
        ...
        with patch(
            "openspider.agents.hooks.bootstrap.is_first_user_interaction",
            return_value=False,
        ):
            result = await hook(mock_agent, {})
        assert result is None
...
class TestBootstrapHookCallHappyPath:
    ...
    async def test_prepends_guidance_to_first_user_message(
        ...
        with patch(
            ...
            "openspider.agents.hooks.bootstrap.build_bootstrap_guidance",
            return_value="guidance text",
        ) as mock_build, patch(
            "openspider.agents.hooks.bootstrap.prepend_to_message_content",
        ) as mock_prepend:
            result = await hook(mock_agent, {})
