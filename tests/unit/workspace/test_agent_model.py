...
def mock_agent_workspace(tmp_path, monkeypatch):
    ...
    import json
    from openspider.config.utils import get_config_path
    from openspider.config.config import Config, AgentsConfig, AgentProfileRef

    # Setup workspace directory
    workspace_dir = tmp_path / "workspaces" / "test_agent"
    workspace_dir.mkdir(parents=True, exist_ok=True)

    monkeypatch.setattr("openspider.config.utils.WORKING_DIR", tmp_path)
    monkeypatch.setattr("openspider.config.config.WORKING_DIR", tmp_path)
