...
from openspider.cli.channels_cmd import channels_group
from openspider.cli.cron_cmd import cron_group
from openspider.cli.daemon_cmd import daemon_group
from openspider.cli.chats_cmd import chats_group
from openspider.cli.skills_cmd import skills_group
...
def test_channels_list_default_agent(
    ...
    """Test openspider channels list uses default agent by default."""
    ...
    with patch("openspider.cli.channels_cmd.load_agent_config") as mock_load:
...
def test_channels_list_custom_agent(
    ...
    """Test openspider channels list with custom agent_id."""
...
def test_cron_list_with_agent_id():
    ...
    with patch("openspider.cli.cron_cmd.client") as mock_client:
...
def test_daemon_status_default_agent():
    ...
    with patch("openspider.cli.daemon_cmd.run_daemon_status") as mock_status:
        with patch("openspider.cli.daemon_cmd._get_agent_workspace") as mock_ws:
...
def test_skills_list_default_agent():
    ...
    with patch(
        "openspider.cli.skills_cmd._get_agent_workspace",
    ) as mock_ws:
        with patch("openspider.cli.skills_cmd.SkillService") as mock_service:
            mock_ws.return_value = "/tmp/default"
...
def test_chats_list_with_agent_id():
    """Test openspider chats list with --agent-id."""
    runner = CliRunner()

    with patch("openspider.cli.chats_cmd.client") as mock_client:
        mock_response = MagicMock()
