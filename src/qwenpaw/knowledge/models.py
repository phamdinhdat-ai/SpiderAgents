...
class DocumentScope(str, Enum):
    """Access scope for a knowledge document."""
...
class KnowledgeDocument(BaseModel):
    ...
    owner: Optional[str] = Field(
        default=None,
        description="Username of the document owner (None = legacy/unowned)",
    )
    scope: DocumentScope = Field(
        default=DocumentScope.PRIVATE,
        description="Access scope: private, shared, or public",
    )
    shared_with: list[str] = Field(
        default_factory=list,
        description="Usernames with explicit access when scope is shared",
    )
