...
def get_openspider_version() -> str:
    """Return the installed OpenSpider package version, or ``'unknown'``."""
    try:
...
def generate_backup_id() -> str:
    ...
    Format: ``openspider-{version}-{YYYYMMDDTHHmmssZ}-{short8}``

    Example: ``openspider-1.2.3-20260420T093000Z-ab12cd34``
    """
    ver = re.sub(r"[^a-zA-Z0-9._-]", "_", get_openspider_version())
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    short = uuid4().hex[:8]
    return f"openspider-{ver}-{ts}-{short}"
...
def finalize_backup_meta(meta, agent_count: int) -> None:
    ...
    meta.openspider_version = get_openspider_version()
