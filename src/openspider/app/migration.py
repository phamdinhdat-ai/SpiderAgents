...
_DEFAULT_AGENT_DESCRIPTION = "Default OpenSpider agent"
...
def _do_migrate_legacy_workspace() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default OpenSpider agent (migrated from legacy config)",
...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.openspider or WORKING_DIR)
...
def _apply_legacy_qa_disable_for_migration(config) -> None:
    ...
    if getattr(ref, "enabled", True):
        ...
        logger.info(
            ...
            "(new OpenSpider builtin QA slot was created)",
            legacy_id,
        )
