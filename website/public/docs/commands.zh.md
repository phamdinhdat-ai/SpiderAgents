...
- 🖼️ - 支持图片输入
- 🎥 - 支持视频输入
- _(user-added)_ - 用户手动添加的模型（通过 `openspider models add-model` 命令）
...
控制和监控 openspider 运行状态的命令，无需通过 Agent 理解意图，直接执行。

可在对话中发送 `/daemon <子命令>` 或短名（如 `/status`），也可在终端执行 `openspider daemon <子命令>`。
...
| `/daemon logs` 或 `/daemon logs 50` | 查看最近 N 行日志（默认 100 行，最大 2000 行，来自工作目录下 `openspider.log`） | ✅   | ✅   |
...
```
/restart                   # 在对话中
openspider daemon restart       # 在终端（仅打印说明）
```
...
openspider daemon reload-config      # 在终端
```
...
显示 openspider 版本号、工作目录路径、日志文件路径。

**用法：**

```
/daemon version            # 在对话中
openspider daemon version       # 在终端
```
...
查看工作目录下 `openspider.log` 的最近 N 行日志。默认 100 行，最大 2000 行。
...
openspider daemon logs -n 200   # 在终端指定 200 行
```
...
每个 mission 在 `~/.openspider/workspaces/default/missions/mission-<timestamp>/` 下创建工作目录：
