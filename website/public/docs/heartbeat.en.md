...
In openspider, **heartbeat** means: on a fixed interval, ask openspider the
“questions” you wrote in a file, and optionally send the openspider’s reply to
**the channel where you last chatted**. Good for “regular check-ins, daily
digests, scheduled reminders” — openspider runs without you sending a
message.
...
1. In the current agent’s workspace there is a **heartbeat query file** (default
   name **HEARTBEAT.md**; rename with env **`openspider_HEARTBEAT_FILE`**). Its
   content is **what to ask openspider on each run** (one or more paragraphs; openspider
...
2. When **`enabled` is true** in config, the system runs on your **every**
   ...
   the user message → openspider replies.
3. **Whether the reply goes to a channel** is set by **target**:
   - **main** — Run openspider only; don’t send the reply to any channel (e.g. local
     self-check, logs).
   - **last** — Send the reply to the **channel/session where you last talked
     to openspider** (e.g. if you last used DingTalk, the heartbeat reply goes to
     DingTalk).
...
`<openspider_WORKING_DIR>/workspaces/<agent_id>/HEARTBEAT.md`.
Default `openspider_WORKING_DIR` is `~/.openspider` (override with **`openspider_WORKING_DIR`**);
`<agent_id>` is the current agent id (e.g. `default`).

The default filename is `HEARTBEAT.md`; use **`openspider_HEARTBEAT_FILE`** to change
...
If you ran `openspider init` without `--defaults`, you may be prompted to edit
...
Example (heartbeat on, openspider only, no channel, every 30m) — in that agent’s
**`agent.json`**:
...
> [CLI](./cli) `openspider cron create` for cron jobs, not heartbeat.
