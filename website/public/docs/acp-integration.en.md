...
openspider supports **ACP (Agent Client Protocol)** in two complementary ways:

1. **openspider using ACP as a Tool**: openspider connects to external ACP runners and uses them as delegated collaborators
2. **openspider as an ACP Server**: external clients connect to openspider over ACP

This page explains both modes and the scenarios each one fits best.

---

## openspider using ACP as a Tool

In this mode, openspider acts as an **ACP client / orchestrator**, connecting to **configured and enabled external ACP runners** and bringing them into the current session as delegated collaboration capabilities.

The actual entry point for this mode is the built-in tool `delegate_external_agent`. It is intended for scenarios where openspider needs to collaborate with other ACP-capable external agent runtimes, such as the built-in examples `opencode`, `qwen_code`, `claude_code`, and `codex`. For more ACP-compatible agents, see the official ACP agent list and integration guide: <https://agentclientprotocol.com/get-started/agents>. In other words, openspider does not directly talk to arbitrary external agents. It talks to runners that have been registered in ACP configuration, and it starts, continues, responds to, and closes delegated collaboration sessions through them.

### What this mode does

In this mode, openspider uses the built-in `delegate_external_agent` tool to:
...
Conceptually, this lets openspider treat an external agent as a collaborative, tool-like capability while keeping openspider as the primary orchestrator of the main conversation.
...
Once the command-line side is ready, you can configure a custom runner in openspider or collaborate with one of the built-in runners directly.
...
When an external ACP runner asks for permission, openspider does **not** decide on the user's behalf.
...
This keeps delegated ACP execution aligned with the same user-controlled safety model used elsewhere in openspider.

### When to use ACP as a Tool

Use this mode when:

- you want openspider to collaborate with another agent runtime
- you have a specialized ACP-compatible external runner for a certain class of tasks
- you want openspider to remain the primary orchestrator while delegating part of the work outward

### ACP Tool vs MCP

ACP as a Tool and MCP solve different problems:

- **MCP** connects openspider to external services and tool servers
- **ACP as a Tool** connects openspider to an external **agent** runtime
...
## openspider as an ACP Server

In this mode, openspider exposes itself as an [Agent Client Protocol (ACP)](https://github.com/agentclientprotocol/python-sdk) compliant agent service over stdio JSON-RPC. External clients, such as [Zed](https://zed.dev), [OpenCode](https://github.com/nicholasgasior/opencode), or any ACP-compatible editor, can connect to openspider via the `openspider acp` command and interact with it programmatically.

### Quick Start

```bash
# Start openspider as an ACP agent
openspider acp

# Use a specific agent profile
openspider acp --agent mybot

# Use a custom workspace directory
openspider acp --workspace /path/to/workspace

# Enable debug logging to stderr
openspider acp --debug
```
...
2. **WORKING_DIR config**: read `agents.active_agent` from `config.json` inside `WORKING_DIR` (default `~/.openspider`, or `~/.openspider` for legacy installations; overridable via the `openspider_WORKING_DIR` environment variable)
...
| Aspect               | openspider as an ACP Server                      | openspider using ACP as a Tool                            |
| -------------------- | --------------------------------------------- | ------------------------------------------------------ |
| openspider's role       | Server / target agent                         | Client / orchestrator                                  |
| Connection direction | External client connects to openspider           | openspider connects to an external runner                 |
| Main purpose         | Let editors or external clients drive openspider | Let openspider delegate work to another agent             |
| Typical entry point  | `openspider acp`                                 | Delegation tool + ACP runner configuration             |
...
ACP in openspider is not just one feature. It supports both directions:

- **Expose openspider outward** as an ACP server
- **Reach outward from openspider** to external ACP agents as delegated tools

If you are integrating openspider into another client, start with **ACP Server**.
If you want openspider to coordinate with another agent runtime, use **ACP as a Tool**.
