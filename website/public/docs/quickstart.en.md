...
This section describes multiple ways to install openspider:
...
on Windows). This installs the `openspider` command.
...
To overwrite existing config, use `openspider init --force` (you will be prompted).
...
channels, openspider will reply there. Otherwise, you can complete this section
...
Then open a new terminal (the installer adds openspider to your PATH automatically).
...
>      - Check if `uv` is available: Enter `uv --version` in CMD. If a version number appears, **only configure the openspider path**. If you receive `'uv' is not recognized as an internal or external command, operable program or batch file,` configure both paths.
...
>      - openspider path: Typically `%USERPROFILE%\.openspider\bin`.
...
To upgrade, simply re-run the install command. To uninstall, run `openspider uninstall`.
...
Images are on **Docker Hub** (`agentscope/openspider`). Image tags: `latest` (stable);
`pre` (PyPI pre-release). Also available on Alibaba Cloud ACR for users in China:
`agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/openspider` (same tags).
...
memory, and skills are stored in the `openspider-data` volume; model configurations
and API keys are stored in the `openspider-secrets` volume; backup archives are stored in the `openspider-backups` volume. To pass API keys, add
...
To deploy openspider on Alibaba Cloud, use the ECS one-click deployment:

1. Open the [openspider Alibaba Cloud ECS deployment link](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) and fill in the deployment parameters as prompted;
...
For detailed steps and instructions, see [Alibaba Cloud Developer Community: Deploy your AI assistant with openspider in 3 minutes](https://developer.aliyun.com/article/1713682).

---

## Option 5: ModelScope Studio one-click setup (no installation)

If you don't want to install Python locally, you can deploy openspider to the cloud
through ModelScope Studio:

1. First go to [ModelScope](https://modelscope.cn/register?back=%2Fhome) to register and log in;
2. Open [openspider Studio](https://modelscope.cn/studios/fork?target=AgentScope/openspider) and complete the one-click setup.

**Important**: Set your Studio to **non-public**, or others may control your openspider.
...
openspider's desktop application without manually configuring Python environments
or running commands.
...
1. **Download the installer**
   Go to [GitHub Releases](https://github.com/agentscope-ai/openspider/releases) to download the version for your system:
...
2. **Install and launch**
   ...
   - **macOS**: Extract the `.zip` to get `openspider.app`, first time requires right-click and select "Open" to bypass system security restrictions
...
openspider needs a large language model to work. You can choose either option:
...
- openspider Local (llama.cpp): download `llama.cpp` inside openspider Local provider settings, see [Models - Local providers Configuration](./models) for details.
...
- For openspider Local (llama.cpp), you can download models directly from the provider settings in the Console, or manually place GGUF model files in the local models directory (default `~/.openspider/local_models/models/<org>/<model>`, for example: `~/.openspider/local_models/models/Qwen/Qwen3-0.6B-GGUF`).
- For Ollama and LM Studio, you need to add models in their respective services first, then openspider can automatically fetch the model list and connect to them.
...
page to test functionality and confirm openspider can reply normally.
...
Chat with openspider in DingTalk, Feishu, QQ, Discord, iMessage, and other apps:
...
4. After saving, you can send messages to openspider in the corresponding app

#### 🔧 Enable and extend skills

Give openspider more capabilities (PDF processing, Office documents, news summaries, etc.):
...
Let openspider execute tasks automatically:

- **Scheduled tasks**: Create in Console **Control → Scheduled tasks**, or use `openspider cron` command in [CLI](./cli)
