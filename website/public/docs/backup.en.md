...
**Backup & Restore** lets you create snapshots of your openspider instance: visually create, export, import, and restore your entire agent environment. It is designed for scenarios such as **rolling back before a version upgrade, migrating between machines, or keeping a safety net before risky changes**.
...
A backup is a single zip file (stored at `~/.openspider.backups/<backup_id>.zip`) that may contain up to four kinds of content:

| Module               | Path                                | Actual content                                                                                                                                                                        |
| -------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Agent workspaces** | `~/.openspider/workspaces/<agent_id>/` | Every file inside each agent's workspace, e.g. persona files, memory, skills, chat history, and channel configs (including channel credentials such as `bot_token` and `app_secret`). |
| **Global settings**  | `~/.openspider/config.json`            | Runtime parameters, security rules, and other global settings.                                                                                                                        |
| **Skill pool**       | `~/.openspider/skill_pool/`            | The globally shared skill repository.                                                                                                                                                 |
| **Secrets**          | `~/.openspider.secret/`                | **LLM provider configuration (including API keys)**, plus environment variables used by tools and skills.                                                                             |
...
A backup ID has the format `openspider-<version>-<timestamp>-<short8>`, which makes it easy to identify the source version and creation time across machines.
...
2. Tick what you need:
   ...
   - **Secrets**: whether to include the secrets (i.e. the `~/.openspider.secret/` directory). Off by default; turning it on shows the same red sensitive-content warning.
...
When you click the **Restore** button on any backup row, openspider first opens the **Pre-restore backup** dialog:
...
- **Pick agents one by one**: restore only the agents you tick; agents outside the restore scope are kept as they are. You can also specify the default storage location for newly added agents at restore time; if unspecified, they go to `~/.openspider/workspaces/<agent_id>/`.
...
| Item             | Path / Default                 |
| ---------------- | ------------------------------ |
| Backup directory | `~/.openspider.backups/`          |
...
Add `-v openspider-backups:/app/working.backups` to your `docker run` command:
...
A: openspider shows an overwrite confirmation; confirm it to continue importing and overwrite the existing backup.
