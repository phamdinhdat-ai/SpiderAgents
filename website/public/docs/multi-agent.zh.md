...
openspider 支持**多智能体**，允许您在同一个 openspider 实例中运行多个独立的 AI 智能体。
...
简单来说，**多智能体**就是让您可以在一个 openspider 中运行多个"分身"，每个分身：
...
启动 openspider 后，在控制台**左上角**可以看到**智能体切换器**：
...
**注意**：删除后工作区目录会保留（防止误删数据），如需彻底清理，请手动删除 `~/.openspider/workspaces/{agent_id}` 目录。
...
如果您之前使用 openspider **v0.0.x**，升级到 **v0.1.0** 时会**自动迁移**：
...
- `openspider channels` - 频道管理
- `openspider cron` - 定时任务
- `openspider daemon` - 运行状态
- `openspider chats` - 对话管理
- `openspider skills` - 技能管理

**不支持 `--agent-id` 的命令**（全局操作）：

- `openspider init` - 初始化
- `openspider providers` - 模型提供商
- `openspider models` - 模型配置
- `openspider env` - 环境变量
