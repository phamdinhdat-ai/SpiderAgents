...
- **Working directory:** Edit skill files directly under `$openspider_WORKING_DIR`
  (default `~/.openspider`), including `$openspider_WORKING_DIR/skill_pool/` and each
  workspace's `$openspider_WORKING_DIR/workspaces/{agent_id}/skills/`.
...
openspider skills are organized in two layers:

- **Skill Pool:** Shared local repository at `$openspider_WORKING_DIR/skill_pool/`
  (default `~/.openspider/skill_pool/`).
- **Workspace Skills:** The local runtime copy at
  `$openspider_WORKING_DIR/workspaces/{agent_id}/skills/`
  (default `~/.openspider/workspaces/{agent_id}/skills/`).
...
- **Conflict handling:** If save, import, upload, or broadcast would land on a
  name that already exists, openspider returns a conflict instead of silently
...
1. **Import built-ins**.
   ...
   | **QA_source_index**           | Internal openspider source/doc index skill for quickly mapping keywords to source paths and local docs.                            | Built-in                                                       |
   | **cron**                      | Scheduled jobs. Create, list, pause, resume, or delete jobs via `openspider cron` or Console **Control → Cron Jobs**.              | Built-in                                                       |
   ...
   | **guidance**                  | Answer openspider installation and configuration questions by consulting local docs first.                                         | Built-in                                                       |
   ...
   **out-of-date**. Use **Update Built-in Skills** to add missing built-ins
   ...
   [CLI](./cli) (`openspider cron`) or Console **Control → Cron Jobs**:

   - Create: `openspider cron create --type agent --name "xxx" --cron "0 9 * * *" ...`
   - List: `openspider cron list`
   - Check state: `openspider cron state <job_id>`
...
6. **Manual filesystem changes**.
   You can place folders directly under `$openspider_WORKING_DIR/skill_pool/`, but this is not
...
`$openspider_WORKING_DIR/workspaces/{agent_id}/skills/`. Those copies are what the agent
...
`$openspider_WORKING_DIR/workspaces/{agent_id}/skills/`, including using openspider itself to help
...
Create a directory under `$openspider_WORKING_DIR/workspaces/{agent_id}/skills/`, add a
...
variables, declare them in `metadata.requires`; openspider exposes them as
...
workspace and channel, openspider injects that config into the runtime environment
...
The full config is always available as `openspider_SKILL_CONFIG_<SKILL_NAME>`
...
- `timeout` is not in `requires.env`, so it is only available via the full
  JSON below.
- `openspider_SKILL_CONFIG_MY_SKILL` always contains the full JSON config.
...
For `requires` metadata, the parser checks keys in order: `metadata.openclaw.requires` → `metadata.openspider.requires` → `metadata.requires`. The first one found is used.
