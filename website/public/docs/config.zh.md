...
openspider 的所有配置和数据都存储在**工作目录**中。本页说明：

- **目录结构** — 文件都在哪里，各目录的作用
- **环境变量** — 如何用环境变量自定义路径和行为
- **配置文件** — `config.json` 和 `agent.json` 的完整字段说明

从 **v0.1.0** 开始，openspider 支持**多智能体**，配置分为两层：
...
默认工作目录是 `~/.openspider`。运行 `openspider init` 后的完整结构：
...
$openspider_SECRET_DIR/                       # 默认 ~/.openspider.secret
├── providers.json                       # 模型提供商配置与 API Key
└── envs.json                            # 环境变量
```

> **路径说明：** `$openspider_WORKING_DIR` 和 `$openspider_SECRET_DIR` 是环境变量，默认值分别为 `~/.openspider` 和 `~/.openspider.secret`。可通过环境变量自定义，详见下方"环境变量"章节。
...
| 变量                       | 默认值              | 说明                                                                                        |
| -------------------------- | ------------------- | ------------------------------------------------------------------------------------------- |
| `openspider_WORKING_DIR`      | `~/.openspider`        | 工作目录根路径                                                                              |
| `openspider_SECRET_DIR`       | `~/.openspider.secret` | 敏感数据目录（存放 `providers.json` 和 `envs.json`）。Docker 中默认为 `/app/working.secret` |
| `openspider_CONFIG_FILE`      | `config.json`       | 配置文件名（相对于 `openspider_WORKING_DIR`）                                                  |
| `openspider_HEARTBEAT_FILE`   | `HEARTBEAT.md`      | 心跳文件名（相对于智能体工作区）                                                            |
| `openspider_JOBS_FILE`        | `jobs.json`         | 定时任务文件名（相对于智能体工作区）                                                        |
| `openspider_CHATS_FILE`       | `chats.json`        | 对话历史文件名（相对于智能体工作区）                                                        |
| `openspider_TOKEN_USAGE_FILE` | `token_usage.json`  | Token 消耗记录文件名（相对于智能体工作区）                                                  |

**其他配置：**

| 变量                                 | 默认值         | 说明                                                            |
| ------------------------------------ | -------------- | --------------------------------------------------------------- |
| `openspider_LOG_LEVEL`                  | `info`         | 日志级别（`debug` / `info` / `warning` / `error` / `critical`） |
| `openspider_MEMORY_COMPACT_THRESHOLD`   | `100000`       | 触发记忆压缩的字符阈值                                          |
| `openspider_MEMORY_COMPACT_KEEP_RECENT` | `3`            | 压缩后保留的最近消息数                                          |
| `openspider_MEMORY_COMPACT_RATIO`       | `0.7`          | 触发压缩的阈值比例（相对于上下文窗口大小）                      |
| `openspider_CONSOLE_STATIC_DIR`         | _（自动检测）_ | 控制台前端静态文件路径                                          |

**安全与认证：**

| 变量                         | 默认值  | 说明                                     |
| ---------------------------- | ------- | ---------------------------------------- |
| `openspider_AUTH_ENABLED`       | `false` | 是否启用 Web 控制台登录认证              |
| `openspider_AUTH_USERNAME`      | -       | 自动注册时的管理员用户名（可选）         |
| `openspider_AUTH_PASSWORD`      | -       | 自动注册时的管理员密码（可选）           |
| `openspider_TOOL_GUARD_ENABLED` | `true`  | 是否启用工具守卫                         |
| `openspider_SKILL_SCAN_MODE`    | `warn`  | 技能扫描模式（`block` / `warn` / `off`） |
...
1. **全局配置** - `~/.openspider/config.json`（提供商、环境变量、智能体列表）
2. **智能体配置** - `~/.openspider/workspaces/{agent_id}/agent.json`（每个智能体的独立配置）
  ...
  "agents": {
    ...
    "profiles": {
      "default": {
        ...
        "enabled": true,
        "workspace_dir": "~/.openspider/workspaces/default"
      }
    }
  },
  "last_api": {
...
| `last_api.host`       | string \| null | `null`         | 上次 `openspider app` 启动的主机地址                |
| `last_api.port`       | int \| null    | `null`         | 上次 `openspider app` 启动的端口                    |
...
| `workspace_dir` | string | 否   | 工作区路径（可选，默认为 `$openspider_WORKING_DIR/workspaces/{id}`） |
...
> **模型提供商配置** 存储在 `$openspider_SECRET_DIR/providers.json`（默认 `~/.openspider.secret/providers.json`）。
> **环境变量配置** 存储在 `$openspider_SECRET_DIR/envs.json`（默认 `~/.openspider.secret/envs.json`）。

### 智能体配置 agent.json

每个智能体在其工作区目录（`$openspider_WORKING_DIR/workspaces/{agent_id}/`）下有独立的 `agent.json`，用于存储该智能体的所有配置（频道、工具、心跳、MCP、安全等）。这样不同智能体可以有完全不同的配置，互不干扰。
...
openspider 需要 LLM 提供商才能运行。配置存储在 `$openspider_SECRET_DIR/providers.json`（默认 `~/.openspider.secret/providers.json`）。

有三种设置方式：

- **`openspider init`** — 交互式向导，最简单
- **控制台 UI** — 在设置 → 模型页面配置
...
| 提供商                                  | ID                       | 说明                          |
| --------------------------------------- | ------------------------ | ----------------------------- |
| openspider Local                           | `openspider-local`          | 本地 llama.cpp 后端           |
...
> **提示：** 运行 `openspider init` 跟着提示走就行——它会列出每个提供商的可用模型让你直接选。
...
- **`openspider init`** — 初始化时会问 "Configure environment variables?"
- **控制台 UI** — 在设置页面编辑
- **API** — `GET/PUT/DELETE /envs`

设置好的变量会在应用启动时自动加载，所有工具和子进程都可以通过 `os.environ` 读取。

> **注意：** 环境变量的值（如第三方 API Key）的有效性需要用户自行保证。openspider 只负责存储和注入，不会校验其正确性。
...
- **`$openspider_WORKING_DIR/skill_pool/`** — 本地共享技能池
- **`$openspider_WORKING_DIR/workspaces/{agent_id}/skills/`** — 智能体工作区中的本地技能

每个技能是一个包含 `SKILL.md` 文件的子目录。技能的启用状态和配置存储在 `skill.json` 文件中（如 `~/.openspider/workspaces/default/skill.json`）。
...
- **`openspider skills config`** — CLI 交互式切换
...
- 默认一切都在 **`$openspider_WORKING_DIR`**（默认 `~/.openspider`）；可通过环境变量自定义。
- 从 **v0.1.0** 开始，配置分为两层：
