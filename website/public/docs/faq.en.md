...
### openspider vs OpenClaw: Feature Comparison

Please check the [Comparison](/docs/comparison) page for detailed feature comparison.

### How to install openspider

openspider supports multiple installation methods. See
[Quick Start](https://openspider.agentscope.io/docs/quickstart) for details:
...
>      - Check if `uv` is available: Enter `uv --version` in CMD. If a version number appears, **only configure the openspider path**. If you receive the prompt `'uv' is not recognized as an internal or external command, operable program or batch file,` configure both paths.
...
>      - openspider path: Typically located at `%USERPROFILE%\.openspider\bin`.
...
> - **Configure `uv` environment variables**: Add the `uv` directory and `%USERPROFILE%\.openspider\bin` to your system's `Path` variable.
> - **Re-run the installation**: Open a new terminal and execute the installation script again to complete the `openspider` installation.
> - **Configure the `openspider` environment variable**: Add `%USERPROFILE%\.openspider\bin` to your system's `Path` variable.

### How to update openspider
...
5. If using the Desktop app (Windows `.exe` or macOS `.zip`), you currently need to uninstall and reinstall:
   - Uninstall openspider on your machine
   - Download the latest build: https://openspider.agentscope.io/downloads
   - Reinstall

After upgrading, restart the service with `openspider app`.

If you previously used CoPaw, upgrading to openspider only requires downloading the latest openspider. No extra migration is needed; your configuration, memory, skills, and other data from the CoPaw era continue to work.

### How to initialize and start openspider service
...
[Quick Start](https://openspider.agentscope.io/docs/quickstart).

### Port 8088 conflict on Windows

On Windows, Hyper-V and WSL2 may reserve certain port ranges, which can conflict
with openspider's default port **8088**. This affects all installation methods
...
- openspider fails to start, or browser cannot connect to `http://127.0.0.1:8088/`
...
1. Run `openspider app --port 8090` from a terminal instead
...
### APITimeoutError when running openspider in WSL2 (NAT mode)

When running openspider inside WSL2 with NAT networking (especially when a VPN is
...
After this, openspider should be able to communicate with model providers
normally.

### Open-source repository

openspider is open source. Official repository:
`https://github.com/agentscope-ai/openspider`

### Where to check latest version upgrade details

See the site [Release notes](https://openspider.agentscope.io/release-notes/?lang=en)
or openspider GitHub [Releases](https://github.com/agentscope-ai/openspider/releases).

### How to configure models

In Console, go to **Settings → Models** to configure. See the
[Models](https://openspider.agentscope.io/docs/models) doc for details:
...
You can also use `openspider models` for setup, downloads, and switching. See
[CLI → Models and environment variables → openspider models](https://openspider.agentscope.io/docs/cli#openspider-models).

### How to use openspider-Flash series models

openspider-Flash is a family of models tuned by the openspider team for openspider's core
...
openspider-Flash models are currently open-sourced on
...
All built-in local providers in openspider can be used with openspider-Flash models:

#### openspider Local (llama.cpp)

In the openspider Local model interface, simply choose a openspider-Flash model to
...
> openspider Local is still in beta. Compatibility across different devices and
> runtime stability are still being improved. If you run into issues while
> using it, please open an issue on GitHub.
> If openspider Local does not work properly in your environment, we recommend
> deploying openspider-Flash with Ollama or LM Studio first.

#### Ollama

1. Download a quantized openspider-Flash model from
   ...
   [openspider-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/openspider-Flash-4B-Q4_K_M).

   - Download with ModelScope CLI:

     ```bash
     modelscope download --model AgentScope/openspider-Flash-4B-Q4_K_M README.md --local_dir ./dir
     ```
...
Create a text file named `openspider-flash.txt` with the following contents. Replace
`/path/to/your/openspider-xxx.gguf` with the absolute path to the `.gguf` file in
the openspider-Flash model repository you downloaded:
...
4. In openspider model settings, choose the Ollama provider, then automatically load
   the model on the Models page.

#### LM Studio

1. Follow step 1 in the Ollama section above to download an appropriate
   quantized openspider-Flash model.
...
4. In openspider model settings, choose the LM Studio provider, then automatically
   load the model on the Models page.

### When using models deployed with Ollama / LM Studio, why can't openspider complete multi-turn interactions, complex tool calls, or remember earlier instructions?

In most cases, this is not a openspider bug. The root cause is usually that the
model's context length is configured too small.

When you deploy a local model with Ollama or LM Studio, if the model's
`context length` is too low, openspider may show problems such as:
...
- Before running openspider, set the model's `context length` to **at least 32K**
- For more complex tasks, frequent tool calls, or longer conversations, you
  may need a value **higher than 32K**

> ⚠️ **Before running openspider, you must set the context length to 32K or higher**
>
> For local models deployed with Ollama or LM Studio, openspider typically needs a
...
The easiest way to create a cron job is to talk to openspider in the channel where you want the results. For example, say: “Create a scheduled task that reminds me to drink water every five minutes.” You can then see the enabled job in Console.

If a scheduled task does not run as expected, try the following:

1. Confirm that the openspider service is running.
...
6. To verify that the task was created and can run, click **Execute Now**. If it works, you should see the reply in the target channel. You can also ask openspider: “Trigger the ‘drink water reminder’ task I just created.”
...
[Skills](https://openspider.agentscope.io/docs/skills).

### How to configure MCP

Go to **Agent -> MCP** in Console. You can enable/disable/delete/create MCP
clients there. See [MCP](https://openspider.agentscope.io/docs/mcp).
...
openspider supports API keys obtained via DashScope Coding Plan. If it still fails,
please check:
...
[issue](https://github.com/agentscope-ai/openspider/issues) in the openspider GitHub
...
Error: Unknown agent error: AuthenticationError: Error code: 401 - {'error': {'message': "You didn't provide an API key. You need to provide your API key in an Authorization header using Bearer auth (i.e. Authorization: Bearer YOUR_KEY). ", 'type': 'invalid_request_error', 'param': None, 'code': None}, 'request_id': 'xxx'}(Details: /var/folders/.../openspider_query_error_qzbx1mv1.json)

Please upload that file (e.g. `/var/folders/.../openspider_query_error_qzbx1mv1.json`)
and also provide your current model provider, model name, and openspider version.
