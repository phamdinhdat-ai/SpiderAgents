...
openspider uses **MCP (Model Context Protocol)** to connect to external services and provides a suite of **built-in tools** that enable agents to access filesystems, execute commands, browse the web, and more.

---

## Concepts

openspider provides two types of tools for agents:

1. **Built-in Tools**: Ready-to-use tools provided by openspider core, such as file operations, command execution, and browser automation
...
**MCP (Model Context Protocol)** allows openspider to connect to external MCP servers, extending the agent's ability to access filesystems, databases, APIs, and other external resources.
...
openspider supports three JSON formats—choose one:
...
openspider provides a set of ready-to-use built-in tools that agents can directly call to perform various tasks.
...
- openspider will call `delegate_external_agent` when appropriate, establish a continuous conversation with the external agent, and stream progress and results back into the current chat
