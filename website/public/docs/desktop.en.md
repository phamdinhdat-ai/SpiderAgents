# openspider Desktop Application Guide
...
This guide explains how to install and use the openspider Desktop application on Windows and macOS.

[releases]: https://openspider.agentscope.io/downloads
...
1. **Download the installer**
   Download `openspider-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\openspider`
...
#### **openspider Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "openspider Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **openspider Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "openspider Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "openspider Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**

A: Go to Windows Settings → Apps → Installed apps → Find "openspider Desktop" → Uninstall
...
1. **Download the archive**
   Download `openspider-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `openspider.app`

3. **Move to Applications folder (Optional)**
   Drag `openspider.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

openspider is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `openspider.app`
...
2. Scroll down to find a message like:
   _"'openspider' was blocked from use because it cannot verify the developer"_
...
xattr -cr /Applications/openspider.app
```
...
- Double-click `openspider.app` to launch
- The app runs in the background and opens a browser window
- Logs are written to: `~/.openspider/desktop.log`
...
cd /Applications  # or wherever your openspider.app is located

# Set environment variables and launch (isolate packaged env, avoid conflicts)
APP_ENV="$(pwd)/openspider.app/Contents/Resources/env"
PYTHONNOUSERSITE=1 PYTHONPATH= PYTHONHOME="$APP_ENV" "$APP_ENV/bin/python" -m openspider desktop
```
...
1. Check the `~/.openspider/desktop.log` file for errors
...
A: Drag `openspider.app` to the Trash, then delete the `~/.openspider` configuration folder
...
- **GitHub Issues**: [Submit an issue](https://github.com/agentscope-ai/openspider/issues)
- **Packaging documentation**: `scripts/pack/README.md` - Technical details and local build guide
- **Log locations**:
  - Windows: View in Debug mode terminal, or `%USERPROFILE%\.openspider\` directory
  - macOS: `~/.openspider/desktop.log`
