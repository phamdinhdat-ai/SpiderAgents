...
openspider 提供了插件系统，允许用户扩展 openspider 的功能。
...
**说明**：`window.openspider.host` 提供以下共享库，插件无需自行打包：
...
**`window.openspider.modules`**：应用启动时会将 `src/pages/` 下的所有模块自动注册到此对象，插件可通过模块键名访问并替换内部导出
...
class CustomGreetingPlugin {
  ...
  setup(): void {
    ...
    // 替换聊天欢迎语
    mod.configProvider.getGreeting = () => "你好！我是定制版 openspider 👋";
...
1. **只安装可信插件**：插件代码会在 openspider 进程中执行
2. **检查依赖**：确保插件依赖来自可信源
3. **审查代码**：安装前审查插件源代码
4. **离线操作**：插件安装/卸载需要 openspider 离线
...
对于需要修改 openspider 行为的插件（如自定义命令），可以使用 monkey patch：
...
一个为 openspider agents 添加 OpenAI GPT Image 2 图片生成能力的工具插件。

**系统要求：**

- openspider 最低版本：`1.1.5`

**安装方法：**

```bash
# 克隆 openspider 仓库（如果尚未克隆）
