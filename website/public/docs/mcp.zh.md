...
openspider 通过 **MCP（模型上下文协议）** 连接外部服务，并提供一组 **内置工具**，让智能体能够访问文件系统、执行命令、浏览网页等。

---

## 概念说明

openspider 为智能体提供两类工具：

1. **内置工具**：开箱即用，由 openspider 核心提供，如文件操作、命令执行、浏览器自动化等
...
**MCP（模型上下文协议，Model Context Protocol）** 允许 openspider 连接到外部 MCP 服务器，扩展智能体访问文件系统、数据库、API 等外部资源的能力。
...
openspider 支持三种 JSON 格式，选择其一即可：
...
openspider 提供了一组开箱即用的内置工具，智能体可以直接调用这些工具完成各种任务。
...
- openspider 会在合适的时候调用 `delegate_external_agent`，与外部智能体建立连续对话，并将中间进度和结果回传到当前会话中
