...
A **channel** is where you talk to openspider: connect DingTalk and it replies
...
- **Edit `agent.json` directly** — Agent workspace config at `~/.openspider/workspaces/{agent_id}/agent.json`, set `enabled: true` and fill in that platform's credentials. Saving triggers a reload without restarting the app.
...
In your agent's `agent.json` (e.g., `~/.openspider/workspaces/default/agent.json`), find `channels.dingtalk` and fill in the corresponding information, for example:
...
> - `robot_code` is recommended in group scenarios; if empty, openspider falls back to `client_id`.

Save the file; if the app is already running, the channel will reload. Otherwise run `openspider app`.
...
4. Run **`openspider app`** to start openspider
...
> **Note:** Follow this order: Configure App ID/Secret → start `openspider app` → then configure the long connection in the Feishu console. If errors persist, try stopping the openspider service and restarting `openspider app`.
...
Find `channels.feishu` in your agent's `agent.json` (e.g., `~/.openspider/workspaces/default/agent.json`). Only **App ID** and **App Secret** are required (copy from the Feishu console under Credentials & basic info):
...
> **Note:** You can also fill in **App ID** and **App Secret** in the Console UI, but you must restart the openspider service before continuing with the long-connection configuration.
...
3. For openspider to read iMessage data, **Terminal** (or the app you use to run `openspider app`) and **Messages** need **Full Disk Access** (System Settings → Privacy & Security → Full Disk Access).
...
4. Set the iMessage database path. The default is `~/Library/Messages/chat.db`; use this unless you've moved the database. You can configure it in either of these ways:
   ...
   - Or edit the agent workspace `agent.json` (usually at `~/.openspider/workspaces/default/agent.json`):
...
Find `channels.discord` in your agent's `agent.json` (e.g., `~/.openspider/workspaces/default/agent.json`) and fill in the fields, for example:
...
5. In **Developer settings**, get **AppID** and **AppSecret** (ClientSecret) and fill them into config (see below). Add your server’s **IP to the whitelist** — only whitelisted IPs can call the Open API outside sandbox.
   ...
   > **Tip:** If you are using ModelScope Creative Space to deploy openspider, the IP whitelist for QQ channel should be: `47.92.200.108`
...
In your agent's `agent.json` (e.g., `~/.openspider/workspaces/default/agent.json`), find `channels.qq` and set `app_id` and `client_secret` to the values above:
...
The **OneBot** channel connects openspider to [NapCat](https://github.com/NapNeko/NapCatQQ), [go-cqhttp](https://github.com/Mrs4s/go-cqhttp), [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), or any other [OneBot v11](https://github.com/botuniverse/onebot-11) compatible implementation via **reverse WebSocket**.
...
openspider starts a WebSocket server; the OneBot implementation (e.g. NapCat) connects to it as a client:

```
NapCat  ──reverse WS──▶  openspider (:6199/ws)
```
...
3. Go to **Network Config** → **New** → **WebSocket Client** (reverse WS):
   - URL: `ws://<openspider_host>:6199/ws`
   - Access Token: same as `access_token` in openspider config (optional)
...
> **Docker Compose tip:** When running openspider and NapCat in Docker Compose, set the NapCat reverse WS URL to `ws://openspider:6199/ws` (using the service name).
...
> **Note:** Audio and video are received at the channel level, but require openspider's transcription provider (`transcription_provider_type`) to be configured for the LLM to process them. Without transcription, voice messages are shown as placeholders.
...
**Method 2:** Fill in `agent.json` (e.g., `~/.openspider/workspaces/default/agent.json`)
...
| `media_dir`              | string | `~/.openspider/media` | Media files (images, files, etc.) download directory |
| `max_reconnect_attempts` | int    | `-1`               | WebSocket max reconnect attempts (`-1` = unlimited)  |
...
- **Authentication**: On first use, scan a QR code to authorize. The token is automatically persisted to a local file (default `~/.openspider/wechat_bot_token`), so you won't need to scan again on subsequent starts.
...
1. Open the openspider Web Console and go to **Settings → Channels → WeChat Personal (iLink)**.
...
You can also configure directly in the agent workspace `agent.json` (e.g., `~/.openspider/workspaces/default/agent.json`):

```json
"wechat": {
  "enabled": true,
  "bot_token": "your_bot_token",
  "bot_token_file": "~/.openspider/wechat_bot_token",
  "base_url": "",
  "media_dir": "~/.openspider/media",
  "dm_policy": "open",
  "group_policy": "open"
...
| `bot_token_file` | string | `~/.openspider/wechat_bot_token` | Path to persist the token for future runs                                             |
| `base_url`       | string | official default              | iLink API base URL; leave empty to use the official default                           |
| `media_dir`      | string | `~/.openspider/media`            | Directory to save received images and files                                           |
...
WECHAT_BOT_TOKEN_FILE=~/.openspider/wechat_bot_token
WECHAT_MEDIA_DIR=~/.openspider/media
...
Find `channels.telegram` in your agent's `agent.json` (e.g., `~/.openspider/workspaces/default/agent.json`) and fill in the fields, for example:
...
2. Fuzzy match subscription and automatic push
   ...
   Subscribe to the wildcard topic `/server/+/up`. Messages will be automatically pushed to the corresponding topic based on the client's `client_id`. For example, after a client pushes a message to `/server/client_a/up`, openspider will push the message to `/client/client_b/down` after processing.
...
3. Redirected topic push
   ...
   Messages will be pushed to `client/client_b/down` based on the `redirect_client_id` attribute, enabling cross-topic push. In IoT scenarios, with openspider as the core, autonomous message pushing between multiple devices can be achieved according to individual requirements.

---

## Matrix

The Matrix channel connects openspider to any Matrix homeserver using the [matrix-nio](https://github.com/poljar/matrix-nio) library. It supports text messaging in both direct messages and group rooms.
...
Find `channels.matrix` in your agent's `agent.json` (e.g., `~/.openspider/workspaces/default/agent.json`):
...
Save the file; the channel will reload automatically if openspider is already running.
...
The XiaoYi channel connects openspider via **A2A (Agent-to-Agent) protocol** over WebSocket to Huawei's AI assistant platform.
...
The Voice channel enables phone call interactions with openspider via Twilio ConversationRelay, supporting Speech-to-Text (STT) and Text-to-Speech (TTS) for voice-based conversations.

### Prerequisites

1. **Twilio Account**: Register at [Twilio](https://www.twilio.com/) and obtain credentials
2. **Cloudflare Tunnel** (or similar): Expose your local openspider service to the public internet for Twilio webhook callbacks
...
Twilio needs to reach openspider's webhook endpoint via the public internet, so you need to expose your local service.
...
```json
{
  "channels": {
    "voice": {
      ...
      "language": "en-US",
      "welcome_greeting": "Hi! This is openspider. How can I help you?"
    }
  }
}
```
...
After configuration, simply call your Twilio phone number to have a voice conversation with openspider:

1. Dial the phone number
2. After hearing the welcome greeting, start speaking
3. openspider converts speech to text and processes it through the Agent
...
| `welcome_greeting`   | string | `"Hi! This is openspider. How can I help you?"` | Welcome message when call connects           |
...
The SIP channel enables voice conversations with openspider via standard SIP phones and softphones (e.g., Linphone, MicroSIP, IP desk phones). It works entirely on your local network or private infrastructure — no cloud account or public URL required.
...
The fastest way to try SIP. openspider starts a built-in SIP registrar automatically — no Asterisk, FreeSWITCH, or any external server needed.

1. Install:

```bash
pip install "openspider[sip]"
```

2. Start openspider and configure in Console:
...
Open **http://127.0.0.1:8088/** → **Settings → Models**: configure a model provider and API key. Then go to **Control → Channels → SIP**: enable it, fill in your DashScope API Key, and click **Save**. All other fields can be left at their defaults — when `sip_server` is empty, openspider automatically starts a built-in registrar, uses `aliyun` for STT/TTS, and picks a default voice.

openspider will restart the SIP channel automatically. You'll see in the terminal:

```
[SIP] Built-in SIP registrar started on 0.0.0.0:5060
[SIP] Quickstart: register your softphone to <Your-IP>:5060
[SIP] Dial 'sip:agent@<Your-IP>:5060' to talk with openspider!
```
...
3. Open [Linphone](https://www.linphone.org/linphone) (or any SIP softphone) and configure:
   ...
   You should hear the welcome greeting, then speak — openspider will reply!
...
2. Install, start openspider, and configure in Console:

```bash
pip install "openspider[sip,sip-livekit]"
openspider init --defaults
openspider app
```
...
3. Generate a token and join via [LiveKit Meet](https://meet.livekit.io/):
   ...
   - Allow microphone access, then speak — openspider responds!
...
Use Asterisk, FreeSWITCH, or any SIP PBX as the registrar. Set `sip_server` to your PBX address. openspider registers as a SIP extension and receives calls routed by the PBX.
...
```json
{
  "channels": {
    "sip": {
      ...
      "language": "zh-CN",
      "welcome_greeting": "你好，我是openspider"
    }
  }
}
```

When `sip_server` is empty, openspider starts a built-in SIP registrar on port 5060 and the agent registers to it automatically. When `sip_server` is set (e.g., `"192.168.1.100:5060"`), openspider registers to that external server instead.

### LiveKit mode configuration

Production mode delegates SIP/RTP to LiveKit SIP Server — a Go binary that handles NAT traversal, jitter buffering, and codec negotiation. openspider joins LiveKit rooms as an AI participant.
...
3. Start openspider. For SIP phone calls, also set up LiveKit infrastructure with a SIP Trunk and Dispatch Rule (see [LiveKit SIP docs](https://docs.livekit.io/sip/)). For browser-based testing, see the [Quick try](#quick-try-livekit-mode-via-browser-3-minutes-no-sip-phone-needed) section above.
...
2. Start speaking — openspider converts speech to text via streaming STT
...
- **Directory**: Channels under the working dir at `custom_channels/` (default `~/.openspider/custom_channels/`) are loaded at runtime. The manager scans `.py` files and packages (subdirs with `__init__.py`), loads `BaseChannel` subclasses, and registers them by the class’s `channel` attribute.
- **Install**: `openspider channels install <key>` creates a template `<key>.py` in `custom_channels/` for you to edit, or use `--path <local path>` / `--url <URL>` to copy a channel module from disk or the web. `openspider channels add <key>` does the same and also adds a default entry to config (with optional `--path`/`--url`).
- **Remove**: `openspider channels remove <key>` deletes that channel’s module from `custom_channels/` (custom channels only; built-ins cannot be removed). By default it also removes the key from `channels` in `config.json`; use `--keep-config` to leave config unchanged.
- **Config**: `ChannelConfig` uses `extra="allow"`, so any channel key can appear under `channels` in `config.json`. Use `openspider channels config` for interactive setup or edit config by hand.

### HTTP route registration

For channels that require webhook callbacks (e.g., WeChat, Slack, LINE), you can register custom HTTP routes by exporting a `register_app_routes` callable in your module — no changes to openspider's core source required.

At startup, openspider scans modules in `custom_channels/` for a `register_app_routes` export. If found, it is called with the FastAPI `app` instance, allowing the channel to register any routes it needs.
...
def register_app_routes(app):
    ...
    async def echo_callback(request):
        ...
        from openspider.app.channels.base import TextContent
...
**Real-world example**: WeChat ClawBot integration ([PR #2140](https://github.com/agentscope-ai/openspider/pull/2140), [Issue #2043](https://github.com/agentscope-ai/openspider/issues/2043)) uses this mechanism to register the `/api/wechat/callback` route with Tencent's official SDK for message delivery.
