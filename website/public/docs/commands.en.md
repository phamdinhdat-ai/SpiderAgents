...
- 🖼️ - Supports image input
- 🎥 - Supports video input
- _(user-added)_ - User-added model (via `openspider models add-model` command)
...
Commands for controlling and monitoring openspider's runtime status. These commands execute directly without going through the Agent.

Send `/daemon <subcommand>` or short names (e.g., `/status`) in chat, or run `openspider daemon <subcommand>` from the terminal.
...
| `/daemon logs` or `/daemon logs 50` | View last N lines of log (default 100, max 2000, from `openspider.log` in working directory) | ✅   | ✅       |
...
```
/status                    # In chat
openspider daemon status        # From terminal
```
...
```
/restart                   # In chat
openspider daemon restart       # From terminal (prints instructions only)
```
...
openspider daemon reload-config      # From terminal
```
...
Display openspider version number, working directory path, and log file path.

**Usage:**

```
/daemon version            # In chat
openspider daemon version       # From terminal
```
...
View the last N lines of `openspider.log` in the working directory. Default 100 lines, maximum 2000 lines.
...
openspider daemon logs -n 200   # From terminal, specify 200 lines
```
...
```bash
openspider daemon status
openspider daemon restart
openspider daemon reload-config
...
```bash
openspider daemon status --agent-id abc123
openspider daemon version --agent-id abc123
```
...
- Loop dir: ~/.openspider/workspaces/default/missions/mission-20260415-123456
...
Each mission creates a working directory under `~/.openspider/workspaces/default/missions/mission-<timestamp>/`:
