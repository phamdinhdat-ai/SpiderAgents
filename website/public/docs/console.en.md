...
The **Console** is openspider's built-in web interface. After running `openspider app`,
open `http://127.0.0.1:8088/` in your browser to enter the Console.

**In the Console, you can:**

- Chat with openspider in real time
- Enable/disable/configure messaging channels
- View and manage all chat sessions
- Manage scheduled jobs and heartbeat
- Edit openspider's persona and behavior files
- Enable/import skills to extend openspider's capabilities
...
This is where you talk to openspider. It is the default page when the Console opens.
...
button (↑). openspider replies in real time.
...
Create and manage scheduled jobs that openspider runs automatically by time.

![Cron Jobs](https://img.alicdn.com/imgextra/i3/O1CN01WsQvrb1dKthp8QMvd_!!6000000003718-2-tps-3822-2070.png)

**Create a new job:**

> If the cron job fails to be created, please refer to the **Troubleshooting Scheduled (Cron) Tasks** section in the [FAQ](https://openspider.agentscope.io/docs/faq) to identify the cause.

The **simplest way to create a cron job is to chat directly with openspider** and let it handle the creation for you. For example, if you want to receive a reminder to drink water on DingTalk, simply message openspider on DingTalk: "Help me create a cron job to remind me to drink water every 5 minutes." Once created, you can view the new task on the Cron Jobs page in the console.
...
2. Fill in each section:
   ...
   - **Task type & content** — **Text**: send fixed text from **Message content**.
     **Agent**: fill **Request content**; on each run openspider receives the text
...
tick, send the contents of `HEARTBEAT.md` as a user message to openspider, and
...
Edit files that define openspider's persona and behavior — `SOUL.md`, `AGENTS.md`,
`HEARTBEAT.md`, etc. — directly in the browser.

> **Multi-agent:** Starting from **v0.1.0**, openspider supports **multi-agent** mode.
> You can run multiple independent agents in one openspider instance, each with its own
...
Manage skills that extend openspider (e.g. read PDF, create Word, fetch news). More
...
Manage runtime environment variables needed by openspider tools and skills (e.g.
`TAVILY_API_KEY`).
...
> **Note:** Variable validity is your responsibility. openspider only stores and loads
> values.
>
...
> Data is stored in `~/.openspider/token_usage.json`. Override the filename with
> `openspider_TOKEN_USAGE_FILE`. See [Config — Environment variables](./config#environment-variables).
...
- **Transcription backend** — **Off**; **Whisper API** (OpenAI-compatible
  ...
  `pip install 'openspider[whisper]'`).
