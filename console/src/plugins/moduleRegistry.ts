 ...
 * 2. Plugins access and modify module exports via window.OpenSpider.modules
...
class ModuleRegistryImpl implements ModuleRegistry {
  ...
  /**
   * Get all modules (for window.OpenSpider.modules)
   */
...
// Expose to window.OpenSpider.modules (for plugin use)
// Set during initialization
if (typeof window !== "undefined") {
  if (!window.OpenSpider) {
    (window as any).OpenSpider = {};
  }

  // Use Proxy for dynamic access, ensuring plugins always get latest module state
  Object.defineProperty(window.OpenSpider, "modules", {
    get() {
