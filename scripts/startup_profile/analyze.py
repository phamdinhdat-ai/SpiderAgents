...
# -*- coding: utf-8 -*-
"""
OpenSpider Startup Performance Analyzer
...
def parse_import_log(log_file):
    ...
    # Filter OpenSpider and third-party
    openspider = [i for i in imports if "openspider" in i["package"]]
    third_party = [
        i
        for i in imports
    ...
    openspider.sort(key=lambda x: x["cumulative_ms"], reverse=True)
    third_party.sort(key=lambda x: x["cumulative_ms"], reverse=True)

    total_openspider = sum(
        i["cumulative_ms"] for i in openspider if i["package"].count(".") == 1
    )
    total_third_party = sum(i["cumulative_ms"] for i in third_party[:10])

    print(
        f"   ✓ {len(imports)} imports, {len(openspider)} OpenSpider, {len(third_party)} third-party",
    )

    return {
        "openspider_imports": openspider,
        "third_party_imports": third_party,
        "summary": {
            "total_ms": total_openspider + total_third_party,
            "total_openspider_ms": total_openspider,
...
def parse_execution_trace(trace_file):
    ...
    try:
        ...
        openspider_funcs = [f for f in functions if "openspider" in f["function"]]

        print(f"   ✓ {len(functions)} functions, {len(openspider_funcs)} OpenSpider")

        return {
            "openspider_functions": openspider_funcs[:50],
...
def main():  # pylint: disable=too-many-statements
    """Main function."""
    print("=" * 60)
    print("🐾 OpenSpider Startup Performance Analyzer")
    print("=" * 60)
    print()
