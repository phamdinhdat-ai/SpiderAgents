# openspider Installer for Windows (self-contained: includes uv download via GitHub)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
#                            [-Extras "dev,whisper"] [-UvPath PATH]
#
# Installs openspider into ~/.openspider with a uv-managed Python environment.
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$openspiderHome     = if ($env:openspider_HOME) { $env:openspider_HOME } else { Join-Path $HOME ".openspider" }
$openspiderVenv     = Join-Path $openspiderHome "venv"
$openspiderBin      = Join-Path $openspiderHome "bin"
$PythonVersion = "3.12"
$openspiderRepo     = "https://github.com/agentscope-ai/openspider.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[openspider] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[openspider] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[openspider] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
openspider Installer for Windows
...
Environment:
  openspider_HOME            Installation directory (default: ~/.openspider)
"@
    exit 0
}

Write-Host "[openspider] " -ForegroundColor Green -NoNewline
Write-Host "Installing openspider into " -NoNewline
Write-Host "$openspiderHome" -ForegroundColor White
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\openspider\console"
...
$Venvopenspider = Join-Path $openspiderVenv "Scripts\openspider.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing openspider from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\openspider"
        }
        Write-Info "Installing openspider from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "openspider-install-$(Get-Random)"
        try {
            git clone --depth 1 $openspiderRepo $cloneDir
    ...
    $package = "openspider"
    if ($Version) { $package = "openspider==$Version" }

    Write-Info "Installing ${package}${ExtrasSuffix} from PyPI..."
    uv pip install "${package}${ExtrasSuffix}" --python $VenvPython --prerelease=allow --quiet --refresh-package openspider
    if ($LASTEXITCODE -ne 0) { Stop-WithError "Installation failed" }
}

# Verify the CLI entry point exists
if (-not (Test-Path $Venvopenspider)) { Stop-WithError "Installation failed: openspider CLI not found in venv" }

Write-Info "openspider installed successfully"

# Check console availability (for PyPI installs, check the installed package)
if (-not $script:ConsoleAvailable) {
    $consoleCheck = & $VenvPython -c "import importlib.resources, openspider; p=importlib.resources.files('openspider')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
    if ($consoleCheck -eq "yes") { $script:ConsoleAvailable = $true }
}

# ── Step 4: Create wrapper scripts ───────────────────────────────────────────
New-Item -ItemType Directory -Path $openspiderBin -Force | Out-Null

$wrapperPath = Join-Path $openspiderBin "openspider.ps1"
$wrapperContent = @'
# openspider CLI wrapper — delegates to the uv-managed environment.
$ErrorActionPreference = "Stop"

$openspiderHome = if ($env:openspider_HOME) { $env:openspider_HOME } else { Join-Path $HOME ".openspider" }
$RealBin   = Join-Path $openspiderHome "venv\Scripts\openspider.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "openspider environment not found at $openspiderHome\venv"
...
$cmdWrapperPath = Join-Path $openspiderBin "openspider.cmd"
$cmdWrapperContent = @"
@echo off
REM openspider CLI wrapper — delegates to the uv-managed environment.
set "openspider_HOME=%openspider_HOME%"
if "%openspider_HOME%"=="" set "openspider_HOME=%USERPROFILE%\.openspider"
set "REAL_BIN=%openspider_HOME%\venv\Scripts\openspider.exe"
if not exist "%REAL_BIN%" (
    echo Error: openspider environment not found at %openspider_HOME%\venv >&2
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use openspider."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "openspider installed successfully!" -ForegroundColor Green
Write-Host ""

Write-Host "  Install location:  " -NoNewline; Write-Host "$openspiderHome" -ForegroundColor White
...
Write-Host ""
Write-Host "  openspider init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  openspider app"  -ForegroundColor White -NoNewline; Write-Host "        # start openspider"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "openspider uninstall" -ForegroundColor White
