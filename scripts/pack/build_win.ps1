...
$Archive = Join-Path $Dist "openspider-env.zip"
...
$VersionFile = Join-Path $RepoRoot "src\openspider\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "openspider-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "openspider-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "openspider Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var openspider_LOG_LEVEL or default to "info"
if not defined openspider_LOG_LEVEL set "openspider_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\openspider_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.openspider\config.json" (
  "%~dp0python.exe" -u -m openspider init --defaults --accept-security
)
"%~dp0python.exe" -u -m openspider desktop --log-level %openspider_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "openspider Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with openspider_LOG_LEVEL)
if not defined openspider_LOG_LEVEL set "openspider_LOG_LEVEL=debug"
...
echo ====================================
echo openspider Desktop - Debug Mode
echo ====================================
echo Working Directory: %cd%
echo Python: "%~dp0python.exe"
echo PATH: %PATH%
echo PYTHONNOUSERSITE: %PYTHONNOUSERSITE%
echo Log Level: %openspider_LOG_LEVEL%
...
echo [Launch] Starting openspider Desktop with log-level=%openspider_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m openspider desktop --log-level %openspider_LOG_LEVEL%
echo.
echo [Exit] openspider Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "openspider Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\openspider Desktop.bat"
WshShell.Run Chr(34) & batPath & Chr(34), 0, False
Set WshShell = Nothing
"@ | Set-Content -Path $LauncherVbs -Encoding ASCII

# Create openspider.cmd wrapper in env root so "openspider" resolves to this
# instead of Scripts\openspider.exe whose embedded Python path may be stale
# after conda-pack/unpack.
$openspiderCmd = Join-Path $EnvRoot "openspider.cmd"
@"
@"%~dp0python.exe" -u -m openspider %*
"@ | Set-Content -Path $openspiderCmd -Encoding ASCII
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('openspider'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] openspider_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "openspider-Setup-$Version.exe"
