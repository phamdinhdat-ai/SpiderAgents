# OpenSpider Desktop packaging scripts
...
# Output: dist/OpenSpider-Setup-<version>.exe
# Creates two launchers:
#   - OpenSpider Desktop.vbs (silent, no console window)
#   - OpenSpider Desktop (Debug).bat (shows console for troubleshooting)
...
APP_ENV="$(pwd)/dist/OpenSpider.app/Contents/Resources/env"
...
When you **double-click** the .app and nothing appears, the launcher writes stderr/stdout to `~/.OpenSpider/desktop.log`. Inspect that file for errors.
...
When users download the OpenSpider macOS app (e.g. from Releases) as a `.app` (in a zip), macOS may show: *"Apple cannot verify that 'OpenSpider' contains no malicious software"*. The app is not notarized. They can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the OpenSpider app → **Open** → in the dialog click **Open** again. Gatekeeper will allow it; after that double-click works as usual.

- **Allow in System Settings**
  If still blocked, go to **System Settings → Privacy & Security**, find the message like *"OpenSpider was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.

- **Remove quarantine attribute (not recommended for most users)**
  In Terminal: `xattr -cr /Applications/OpenSpider.app` (or the path to the `.app` after unzipping). This clears the download quarantine flag; less safe than right-click → Open.
...
| File | Description |
|------|-------------|
| `build_common.py` | Create temporary conda env, install `OpenSpider[full]` from a wheel, conda-pack; produces archive. |
| `build_macos.sh` | One-click: build wheel → build_common → unpack into OpenSpider.app; optional zip. |
